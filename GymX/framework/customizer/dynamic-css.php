 ...
 * in a way that we have a custom data type `Calluna_Customize_CSS` which is capable
 * of dynamically generate the CSS out from the provided array `$css_map`.
 */

if ( class_exists( 'WP_Customize_Setting' ) && ! class_exists( 'GymX_Customize_CSS' ) ) {
	class GymX_Customize_CSS extends WP_Customize_Setting {
		/*  for PRO users! - *
		 ...
		 * @return 2D array
		 */
		public function gymx_get_css_map() {
			return $this->css_map;
		}
		 ...
		 * @return array
		 */
		public function gymx_get_all_css_properties() {
			return array_keys( $this->gymx_get_css_map() );
		}
		 ...
		 */
		public function gymx_css_selector_groups_for_property( $css_property ) {
			...
			if ( array_key_exists( $css_property , $this->css_map ) ) {
				...
				foreach ( $this->css_map[ $css_property ] as $css_selector ) {
					if ( self::gymx_is_filterable_string( $css_selector ) ) {
						// here were save in $css_selector and $media_query vars the actually useful numbers
						list( $css_selector, $media_query )      = explode( self::LEADX_FILTER_SEPARATOR, $css_selector );
		 ...
		 */
		public function gymx_plain_selectors_for_all_groups( $css_property ) {
			$selectors = array();
			$all_groups = $this->gymx_css_selector_groups_for_property( $css_property );
		 ...
		 */
		public function gymx_setting_render_css() {
			$out = '';

			foreach ( $this->gymx_get_css_map() as $css_prop_raw => $css_selectors ) {
				// we get here the $css_prop and $value
				extract( $this->gymx_filter_css_property( $css_prop_raw ) );

				foreach ( $this->gymx_css_selector_groups_for_property( $css_prop_raw ) as $media_query => $css_selectors_arr ) {
		 ...
		 */
		public function gymx_filter_css_property( $css_property ) {
			// defaults
			$out = array(
			...
			if ( self::gymx_is_filterable_string( $css_property ) ) {
				list( $css_property, $filter ) = explode( self::LEADX_FILTER_SEPARATOR, $css_property );
				...
				if ( preg_match( '/^([a-z0-9\_]+)(?:\((\w+)\))?$/i', trim( $filter ), $matches ) ) {
					switch ( $matches[1] ) {
						case 'darken':
							$out['value'] = $this->gymx_darken_css_color( $this->value(), (int) $matches[2] );
							break;
						case 'lighten':
							$out['value'] = $this->gymx_lighten_css_color( $this->value(), (int) $matches[2] );
							break;
						case 'important':
						...
						case 'linear_gradient_to_bottom':
							$out['value'] = sprintf( '%1$s linear-gradient(to bottom, %1$s, %2$s)', $this->value(), $this->gymx_darken_css_color( $this->value(), (int) $matches[2] ) );
							break;
						default:
		 ...
		 * @return boolean
		 */
		public static function gymx_is_filterable_string( $css_property ) {
			return strpos( $css_property, self::LEADX_FILTER_SEPARATOR ) > 0;
		}
		 ...
		 */
		protected static function gymx_darken_css_color( $color, $percent = 20 ) {
			$alter_for = 2.55 * $percent;

			$parts = self::gymx_rgb_from_hexdec( $color );
		 ...
		 * @return string Hexdec color.
		 */
		protected static function gymx_lighten_css_color( $color, $percent = 20 ) {
			$alter_for = 2.55 * $percent;
		 ...
		 * @return array
		 */
		protected static function gymx_rgb_from_hexdec( $hexdec ) {
			$rgb = array();
