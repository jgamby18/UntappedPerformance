	...
	extract(shortcode_atts(array(
		...
		if( $wp_query->have_posts() ) :
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post();
				...
				if (isset($background)) {
					$out .= '<div class="blog-item" style="background-color:' . esc_attr($background) . ';border-color:' . esc_attr($border) . '">';
				}
				else {
				...
				if ($post_format == 'audio' && get_post_meta( $post->ID, 'gymx_post_blog-audioembed', true ) != '') {
		  		...
		  		if ($post_format != 'quote') {
		  			$out .= '<div class="blog-item-description">
		  					<span class="post-date">'.esc_attr(get_the_date(get_option( 'date_format' ), $post->ID)).'</span>
