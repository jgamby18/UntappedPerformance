...
<section class="wrapper <?php echo esc_attr($top_padding) . ' ' . esc_attr($bottom_padding); ?>" id="single-wrapper">
    ...
    if ( have_posts() ) : while ( have_posts() ) : the_post();
    	$position   = rwmb_meta( 'gymx_post_trainer-position' );
        $twitter    = rwmb_meta( 'gymx_post_trainer-twitter' );
        $facebook   = rwmb_meta( 'gymx_post_trainer-facebook' );
        $googleplus = rwmb_meta( 'gymx_post_trainer-googleplus' );
        $instagram  = rwmb_meta( 'gymx_post_trainer-instagram' );
        $linkedin   = rwmb_meta( 'gymx_post_trainer-linkedin' );
        $dribbble   = rwmb_meta( 'gymx_post_trainer-dribbble' );
        $skype      = rwmb_meta( 'gymx_post_trainer-skype' );
        $phone      = rwmb_meta( 'gymx_post_trainer-phone' );
        $mail       = rwmb_meta( 'gymx_post_trainer-mail' );
        $terms = get_the_terms( get_the_ID(), 'trainers_category' ); ?>
            ...
            <div class="row">
            	<div class="col-md-4">
                   ...
                   // Social Profiles
				    ...
				    <?php
				    //Team Sidebar
				    if ( is_active_sidebar('sidebar-trainers') && rwmb_meta( 'gymx_post_content_sidebar',$post->ID ) != 'hide' ) { ?>
				        <div class="team-member-sidebar">
                    		<?php dynamic_sidebar( 'sidebar-trainers' ); ?>
                    	</div>
				    <?php } ?>
            	</div>
            	<div class="col-md-8">
