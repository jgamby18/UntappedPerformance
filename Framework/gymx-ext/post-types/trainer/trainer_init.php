...
class GymX_Ext_Trainer_Config {
	...
	public function __construct() {
		...
		// Adds the trainers custom sidebar
		add_filter( 'widgets_init', array( $this, 'gym_ext_register_sidebar' ), 10 );
		add_filter( 'gym_ext_get_sidebar', array( $this, 'gym_ext_display_sidebar' ), 11 );
	...
	/**
	 * Registers a new custom trainers sidebar.
	 */
	public static function gym_ext_register_sidebar() {


		// Return if custom sidebar is disabled
		if ( ! get_theme_mod( 'trainer_custom_sidebar', true ) ) {
			return;
		}

		// Get post type object to name sidebar correctly
		$obj            = get_post_type_object( 'trainers' );
		$post_type_name = $obj->labels->name;

		// Register trainers_sidebar
		register_sidebar( array (
			'name'          => $post_type_name .' '. esc_html__( 'Sidebar', 'ttbase-framework' ),
			'id'            => 'sidebar-trainers',
			'before_widget' => '<aside id="%1$s" class="sidebar trainers widget %2$s">',
            'after_widget' => '</aside>',
	...
	/**
	 * Alter main sidebar to display trainers sidebar.
	 *
	 * @since 2.0.0
	 */
	public static function gym_ext_display_sidebar( $sidebar ) {
		if ( get_theme_mod( 'trainers_custom_sidebar', true ) && ( is_singular( 'trainers' ) || gym_ext_is_trainers_tax() ) ) {
			$sidebar = 'trainers_sidebar';
		}
		return $sidebar;
	}
