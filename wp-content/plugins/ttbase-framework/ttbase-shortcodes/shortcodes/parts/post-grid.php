...
function ttbase_framework_posts_grid_shortcode($atts) {
	...
	if ( $ttbase_post_grid_query->posts ) :
		...
		$count++;
			...
			$post_date		  = sprintf(wp_kses(__( '<span class="ttbase-recent-posts-entry-posted-on">%1$s</span>', 'ttbase-framework'), array( 'span' => array( 'class' => array() ) ) ),esc_html( get_the_date(get_option( 'date_format' ), $post_id) ) );
						...
						if ( $excerpt == 'true' ) {
							...
							$output .= '</p>';
								if ( $read_more == 'true' && ( $post_excerpt || $custom_excerpt ) ) { 
									$output .= '<div class="read-more-link-wrapper"><a class="read-more-link" href="'. esc_url($url) .'" title="'. $post_title .'">'. $read_more_text .'<i class="icon-next"></i></a></div>';
								}
