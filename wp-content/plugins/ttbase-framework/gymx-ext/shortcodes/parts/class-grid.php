...
function ttbase_framework_class_grid_shortcode($atts) {
	...
	<?php } //end if showfilter ?>
		<div id="<?php echo intval($randomnumber); ?>" class="ttbase-class-items ttbase-grid <?php echo esc_attr($style); ?> clearfix" data-id="<?php echo intval($randomnumber); ?>">  
			<?php 
			$count=0;
					...
					<div class="ttbase-class-item">
						...
						if ( $excerpt == 'true' && ( $post_excerpt || $custom_excerpt )) { ?>
							...
							<?php if ( $read_more == 'true' && ( $post_excerpt || $custom_excerpt ) ) { ?>
									<div class="read-more-link-wrapper"><a class="read-more-link" href="<?php echo esc_url($url); ?>" title="<?php echo esc_attr($class_name); ?>"><?php echo esc_attr($read_more_text); ?><i class="icon-next"></i></a></div>
							<?php } ?>
							</div>
					<?php } ?>
					</div>
